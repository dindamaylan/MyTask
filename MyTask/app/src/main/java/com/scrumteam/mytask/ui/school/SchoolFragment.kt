...
class SchoolFragment : Fragment() {
        ...
        listTaskAdapter = ListTaskAdapter(
            ...
            onCompleteItem = {
                act.setupBottomSheetCheckedTask {
                    schoolViewModel.checkedTask(it.copy(checked = true))
                }
            }
        )
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        schoolViewModel.schoolState.observe(viewLifecycleOwner) { state ->
            if (!state.isError) {
                val tasks = state.schoolTask
                showMessageState(tasks.isEmpty(), state.message)
                listTaskAdapter.submitList(state.schoolTask)
        ...
        schoolViewModel.schoolCheckedState.observe(viewLifecycleOwner) {
        ...
        if (popoupMenu == null) {
            ...
            popoupMenu.setOnMenuItemClickListener { menuItem: MenuItem ->
                when (menuItem.itemId) {
                    R.id.delete_task -> {
                        act.setupBottomSheetDeleteTask { schoolViewModel.deleteSchoolTask(task) }
                    }
                    R.id.edit_task -> {
